...
Area files are generated by the rules-loop process (`rules-loop.sh`) and are **not** part of a database clone. `areas.bin` and `area_blocks.bin` are documented in `doc/area-format.md`; `area_tags_local.bin` and `area_tags_global.bin` are documented in `doc/tag-data-format.md`.
...
- [x] `areas.bin` / `areas.bin.idx` — Area_Skeleton records: area ID → set of block tile indices (`doc/area-format.md`)
- [x] `area_blocks.bin` / `area_blocks.bin.idx` — Area_Block records: boundary segment endpoints per tile (`doc/area-format.md`)
